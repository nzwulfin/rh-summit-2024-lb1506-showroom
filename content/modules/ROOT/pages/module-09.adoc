...
NOTE: You will need the value of the `REGISTRY` environment variable from the `Terminal` to subsitute in the commands below.
...
In a short time after that command, you should be able to ssh back to the virtual machine via the `SSH ISO VM` tab and check the bootc status:
