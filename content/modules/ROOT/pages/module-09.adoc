...
by listing the file `/var/lib/libvirt/images/summit/rhel-boot.iso`

[source,bash]
----
ls -lah /var/lib/libvirt/images/summit/rhel-boot.iso
----
...
[source,bash]
----
make iso CONTAINER=$REGISTRY/bootc
----

At the end of the run, you should be able to list the file `/var/lib/libvirt/images/summit/rhel-boot-custom.iso`:

----
ls -lah /var/lib/libvirt/images/summit/rhel-boot-custom.iso
----
...
+
----
skopeo copy "docker://$REGISTRY/bootc" "oci:${TEMPDIR}/container/"
----
...
NOTE: This will take 3+ minutes to complete
...
While we wait for the install to finish, let's look at the kickstart file.

NOTE: The Kickstart added to the iso is in `config/local.ks`. You can use the `Code editor` or the `Terminal` as well to view the file.
...
Once you see the following from the console, the install should be complete.
...
NOTE: This may throw hostname errors until the system is fully booted and libvirt-nss can resolve the IP
...
The output should be "Hello Red Hat One 2024!!"

You should also be able to login to the virtual machine via the `SSH ISO VM` tab. 

NOTE: If the ssh key is not automatically picked up, the user password is `lb1506`.
...
To make things easier, create a local `REGISTRY` environment variable in this VM shell using the value from the `Terminal` tab. The value below is an example and will need to match your workshop.

[source,bash]
----
export REGISTRY=rhel.xkxn6lavs9ns.instruqt.io:5000
----
...
[source,bash]
----
sudo skopeo copy --tls-verify=false docker://$REGISTRY/bootc  containers-storage:$REGISTRY/bootc 
----
...
[source,bash]
----
sudo bootc switch --transport containers-storage $REGISTRY/bootc
----

....
Loading usr/lib/ostree/prepare-root.conf
Queued for next boot: ostree-unverified-image:containers-storage:$REGISTRY/bootc
  Version: 9.20240501.0
...
`SSH ISO VM` tab and not the `Terminal` tab.
...
Click on the refresh icon in the right corner of the tab to ssh back to the virtual machine and check the bootc status:
...
spec:
  image:
    image: $REGISTRY/bootc
    transport: containers-storage # <1>
  bootOrder: default
status:
  staged: null
  booted:
...
<1> In the status you can see `bootc` is now tracking local container storage for updates. Further updates would then be provided on media 
