...
You can also use the `Code editor` tab to make the following changes, you can create a new file by clicking the 
button to the right of the directory name `/root/rh-summit-2024-lb1506`.

[source,bash]
----
vim config-qcow2.json
----
...
Return to the `Terminal` tab to create the disk image.

First, check that the bootc image is available to the local registry:

[source,bash]
----
skopeo inspect docker://$REGISTRY/bootc | jq '.Digest'
----
....
...
From here, these arguments passed to `bootc-image-builder`
  ...
  * `$REGISTRY/bootc` -> the bootc image we are unpacking into the qcow2 disk

Once this completes, you will find the image is the `qcow2/` directory:

NOTE: For your convenience, this part of the exercise is automated by using the `make qcow CONTAINER=$REGISTRY/bootc` command.
...
[source,bash]
----
cp qcow2/disk.qcow2 /var/lib/libvirt/images/summit/qcow-vm.qcow2
virt-install --connect qemu:///system \
                --name qcow \
                --disk /var/lib/libvirt/images/summit/qcow-vm.qcow2 \
                --import \
                --network "network=default,mac=de:ad:be:ef:01:03" \
                --memory 4096 \
                --graphics none \
...
You can now login to the virtual machine. Navigate to the `SSH QCOW VM` tab of the interface. You may need to hit the refresh icon in the upper right corner of the tab.
...
<1> This block provides information about the image in use. You can see that image is listed as `$REGISTRY/bootc`.
