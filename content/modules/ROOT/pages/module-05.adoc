...
NOTE: You should still be logged into the qcow VM 
...
NOTE: Return to the tab labeled `Terminal` to procced as the VM reboots.
...
You can use the `Code editor` tab or `vim` in the Terminal to edit the Containerfile to match the following:
...
If you used the `Code editor` to change the Containerfile, be sure to return to the `Terminal` tab now.
...
Now you can return the `SSH QCOW VM` tab and hit refresh to log in via ssh.
...
TIP: Try searching the bash history for 'bootc update' and adding the new flag. Even though we've rolled back the OS, we still have local user state available.
...
You're prompted for a password by `sudo` because we rolled back to a version of the image that does not include our drop-in file.
...
We can check for our new web page from the `Terminal` tab once the VM has rebooted.
...
Now the output should be "Hello Red Hat One 2025!!"
