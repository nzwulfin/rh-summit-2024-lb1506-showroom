...
[source,bash]
----
podman build --secret id=SSHPUBKEY,src=/home/lab-user/.ssh/id_rsa.pub --file Containerfile --tag $REGISTRY/bootc-auth
----
...
We can change to our new image with the embedded `root` credentials, substitute the full hostname from the `Terminal` here.

[source,bash]
----
sudo bootc switch $REGISTRY/bootc-auth
----
