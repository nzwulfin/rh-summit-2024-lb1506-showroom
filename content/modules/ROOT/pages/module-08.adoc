...
In the `Terminal` tab, you will need to have the full registry name for the following commands in the VM ssh session.
...
We can change to our new image with the embedded `root` credentials, substitute the full hostname from the `Terminal` for `your-reg-name` here.
