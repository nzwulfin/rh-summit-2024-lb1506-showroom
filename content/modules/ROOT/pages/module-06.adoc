...
In the `Code editor` tab, select the examples directory and then the Containerfile.wp.
...
[source,bash]
----
podman build --file Containerfile.wp --tag $REGISTRY/bootc-wp
----
...
[source,bash]
----
sudo bootc switch $REGISTRY/bootc-wp
----
