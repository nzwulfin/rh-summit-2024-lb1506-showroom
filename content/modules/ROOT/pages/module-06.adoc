...
Update the `FROM` line in the Containerfile to reflect the registry host in your lab environment: `$REGISTRY/bootc`
[source,dockerfile]
----
...
After a short while, you can log back in to the virtual machine via the `SSH QCOW VM` tab by pressing the refresh icon.
