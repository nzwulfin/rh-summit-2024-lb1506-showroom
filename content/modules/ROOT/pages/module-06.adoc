...
In the `Terminal` tab, you will need to have the full registry name to add to the Containerfile we are about to edit.
...
In the `Code editor` tab file picker, select the **examples/wordpress-quadlet/** directory and then the **Containerfile.wp**.
...
Update the `FROM` line in the Containerfile to reflect the registry host in your lab environment: 

eg `FROM rhel.auwwhe2suwht.instruqt.io:5000/bootc`
...
First, this is a derived image, not directly using the Red Hat base image. This would be how we expect most folks to use image mode for RHEL to create and internally distribute standard operating environments to support applications. The container model of inheritance can be a powerful tool when looking to control drift and improve standardization.

Remember the `ADD` directive pulls full directories into the image at build time. So all of the work here is 
somehow being done in `/usr` or `/etc`, let's see what's in there. You can return to the `Terminal` tab to see the files with the `ls` command below, or use the `Code editor` to review them.
...
NOTE: Be sure you are in the `Terminal` tab for this part of the exercise
...
Tagging is a powerful but somewhat subjective way to both provide information about an image and to control what's visible to any particular image mode host. For example, semantic versioning to track changes, or an environment name to track where images should be deployed. Most of that discussion is out of scope for this lab, but we'll explore how to use a basic alternate image tag here. 
Remember the format for the `--tag` flag is `<registry>/<repository>:<tag>`.

[source,bash]
----
podman build --file examples/wordpress-quadlet/Containerfile.wp --tag $REGISTRY/bootc-wp
----
...
You will need to have the full image name for the next exercise, use `podman` to find the last image built.
...
You can now login to the virtual machine via the `SSH QCOW VM` tab.

NOTE: You may need to click on the refresh icon in the right corner to the tab to log in after the last reboot.
...
Replace the string `your-reg-name` below with the value of the environment variable in the `Terminal` tab:

eg `sudo bootc switch rhel.auwwhe2suwht.instruqt.io:5000/bootc-wp`
[source,bash]
----
sudo bootc switch your-reg-name/bootc-wp
----
...
virtual machine tab and not the `Terminal` (the prompt should look like `[lab-user@localhost ~]#`):
...
After a short wait, you can log back in to the virtual machine via the `SSH QCOW VM` tab by pressing the refresh icon.

[#layers]
== Troubleshooting layered builds

Let's watch our newly created quadlet service start for the first time.
...
After the container is pulled and the service attempts to start you will see:
....
Nov 19 19:02:59 localhost.localdomain caddy[2367]: Error: cannot listen on the TCP port: listen tcp4 :80: bind: address already in use
Nov 19 19:02:59 localhost.localdomain systemd[1]: caddy.service: Main process exited, code=exited, status=126/n/a
Nov 19 19:02:59 localhost.localdomain systemd[1]: caddy.service: Failed with result 'exit-code'.
Nov 19 19:02:59 localhost.localdomain systemd[1]: Failed to start Caddy Quadlet.
Nov 19 19:03:00 localhost.localdomain systemd[1]: caddy.service: Scheduled restart job, restart counter is at 7.
Nov 19 19:03:00 localhost.localdomain systemd[1]: Stopped Caddy Quadlet.
Nov 19 19:03:00 localhost.localdomain systemd[1]: caddy.service: Start request repeated too quickly.
Nov 19 19:03:00 localhost.localdomain systemd[1]: caddy.service: Failed with result 'exit-code'.
Nov 19 19:03:00 localhost.localdomain systemd[1]: Failed to start Caddy Quadlet.
....
...
If you look at the original Containerfile of the source image, you'll recall we set Apache to start at boot:
