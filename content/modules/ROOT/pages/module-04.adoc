...
Before proceeding, make sure you have returned to the `Terminal` tab.
...
These files are also provided in the `examples/update` directory. You should be in `~/rh-summit-2024-lb1506` when running this command.
...
NOTE: We're using `vim` here, but feel free to use the `Code editor` to create these files. Each directory has a 'New file' button
in the file picker pane. It's the first button of the two.
...
Now that we have our new configs, we can add them to the Containerfile and rebuild the image. While we're at it, that index message is a little bland, let's change that too.
...
RUN echo "Hello Red Hat One 2025!!" > /var/www/html/index.html

RUN systemctl enable httpd.service
----
<1> Adding the files we just created

If you aren't familiar, the `ADD` directive has some extra capabilities over the `COPY` command. Where 
...
Now you can return to the tab labeled `SSH QCOW VM`. If you have been logged out, hit the refresh icon in the right corner of the title bar.
...
....
layers already present: 66; layers needed: 4 (157.8 MB)
Fetched layers: 150.52 MiB in 28 seconds (5.45 MiB/s)
...
virtual machine tab and not the Terminal.
...
After a short wait, hit the refresh icon in the upper right corner to log back into the system and and you should see the message of the day after logging in successfully.
