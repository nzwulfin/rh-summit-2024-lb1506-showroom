...
NOTE: You should in the `Terminal` tab for this exercise.
...
[source,bash]
----
podman run --rm --name http-test --detach --publish 80:80 $REGISTRY/bootc
----

Examine the details of our running container.
...
----
2eba42ec8b2d  rhel.hd9zil0aonr2.instruqt.io:5000/bootc:latest  /sbin/init            9 seconds ago   Up 9 seconds   0.0.0.0:80->80/tcp      http-test
----

We exposed a port from the container on the lab host, so we can test the web server with `curl`.
...
We have `systemd` available so we can control services and check status:
...
But since we are in a container, other things like SELinux will not be able to be tested:
